...
/*    void check_step(const std::vector<std::vector <double> >& examples, const std::vector<std::vector <size_t> >& indecies_of_pairs,
        ...
        size_t W_size = idpair._discrete_points_for_all_dims.get_number_of_vertices();
    ...
    std::vector<double> init(const std::vector<std::vector <double> >& examples, const std::vector<std::vector <size_t> >& indecies_of_pairs,
                             const std::vector<short>& tags, const std::vector<std::vector <double> >& discrete_points){
        
    	srand(time(NULL));
        double thold, min_c_error;
        std::vector<double> C;
        std::vector<double> Wreg;
        std::vector<double> W;
        ...
        for (size_t j = 0; j < C.size(); j++) {
		    for (size_t k = 0; k < NUMBER_OF_ITERATIONS; k++) {
    			part_to_70_30(indecies_of_pairs, tags, training, training_tags, testing, testing_tags);   // divide precentage for cross validation
				W = learn_similar(examples, id_pair, training, training_tags, Wreg, C[j], thold);
				
				size_t error = classificationTest(W, examples, id_pair, testing, testing_tags, thold);
    ...
    */
    std::vector<double> learn_similar(const std::vector<std::vector <double> >& examples , const IDpair& idpair , const std::vector<std::vector <size_t> >& indecies_of_pairs ,
                                      const std::vector<short>& tags, const std::vector<double>& Wreg, const std::vector<double>& C, double& thold){
