...
*/
    ...
    std::cout << "dis_points:" << std::endl;
    dis_points.print(std::cout);
    
    stdvecvec dis_points2= l.mat_to_std_vec_2(dis_points);

    // std::cout << "dis_points2:" << std::endl;
    // for (auto i : dis_points2) {
    //     for (auto j : i) {
    //         std::cout << j << " ";
    //     }std::cout << std::endl;
    // }
...
/*    std::cout << "examples:" << std::endl;
    ...
    std::vector<std::vector<size_t> > groups = {{0}, {1}, {2}, {0, 1}, {1, 2}, {0, 2}, {0, 1, 2}};
    GridGroup grid_group(dis_points2, groups, true);
    
    IDgroupPair id_group_pair(grid_group, groups);
    
    for (int i = 0; i < 1/*C.size()*/; i++) {
        std::vector<double> W = l.init_group(examples, pairs, tags, grid_group, groups, 0.1, thold);
        int counter = 0;
        
        for (size_t i = 0; i < pairs.size(); i++) {
            const std::vector<Pair>& vol = id_group_pair( examples[ pairs[i][0] ], examples[ pairs[i][1] ] );
            short s = l.classification(W, vol, thold);
